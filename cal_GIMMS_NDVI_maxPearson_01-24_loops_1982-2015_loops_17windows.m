...
%%%%%%%
cd('D:\Droughts_scales\output')
save(['Scale_max_Pearson_GIMMS_NDVI_Slope_',num2str(window,'%02d'),'.mat'],'scale_slope'); 
save(['Scale_max_Pearson_GIMMS_NDVI_Sig_',num2str(window,'%02d'),'.mat'],'scale_sig'); 
...
%%%%%%%
cd('D:\Droughts_scales\output')
save(['Coef_max_Pearson_GIMMS_NDVI_Slope_',num2str(window,'%02d'),'.mat'],'r_slope'); 
save(['Coef_max_Pearson_GIMMS_NDVI_Sig_',num2str(window,'%02d'),'.mat'],'r_sig'); 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
disp('Start spatial corr(Time-Scale,R)')
warning off
cd('D:\Droughts_scales\output')
load(['Coef_max_Pearson_Rval_GIMMS_NDVI_Spei_t_',num2str(window,'%02d'),'.mat']);
load(['Scale_max_Pearson_Rval_GIMMS_NDVI_Spei_t_',num2str(window,'%02d'),'.mat']);
...
save(['R_spatial_max_Pearson_GIMMS_NDVI_R_',num2str(window,'%02d'),'.mat'],'R'); 
save(['P_spatial_max_Pearson_GIMMS_NDVI_Sig_',num2str(window,'%02d'),'.mat'],'P'); 
...
da(idv~=1)=nan;         
da(ghm~=1)=nan;    
id=find(~isnan(da));a=A(id);da=da(id);b=da.*a;%area-weighted
y(t)=nansum(b(:))/nansum(a(:));
...
da(idv~=1)=nan;         
da(ghm~=1)=nan;    
id=find(~isnan(da));da=da(id);a=A(id);a=a./nanmax(a(:));b=da.*a; %area-weighted
y(t)=nanmean(b(:)) ;
%calculated 95% values of the t-distributio
data=b;
N = size(data,1);                    
